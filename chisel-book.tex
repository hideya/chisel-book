...
%%end novalidate
\vspace*{21mm}
\hfill\includegraphics[width=100mm]{chisel-book-ja/inner-front.pdf}
\fi
...
% Acknowledgement:
% I very much acknowledge the translation of the Chisel book, 2nd edition, from English to Japanese.
% The English version of the book is used in teaching and on several tutorials on Chisel during the year.
% I hope that the Japanese version can serve the same and help to introduce Chisel
% as a hardware construction language for University courses on digital design.
% October 5, 2021.
% Martin Schoeberl
\ifshowtrans
...
「Digital Design with Chisel」の第二版を、英語から日本語に翻訳していただいたことを大変感謝しています。
この本の英語版は、年間を通じてChiselに関する授業やいくつかのチュートリアルで使用されています。
日本語版も同様の役割を果たし、
大学のデジタル回路設計コースのハードウェア構築言語としてChiselを紹介する一助となることを願っています。
...
\myref{http://gtkwave.sourceforge.net/}{GTKWave}と\myref{https://www.jetbrains.com/idea/download/}{IntelliJ}（コミュニティ版）をインストールします。
プロジェクトをインポートする際には、インストール済みの \emph{JDK 1.8を選択} してください（Java 11ではありません！）。
...
（堀江徹也 著）
は、Chiselを使って、FPGAボードをインプリメントし、外部回路を接続する方法などが解説されています。
\end{spacing}%%%
\end{itemize}
\fi %(日本語版追記)
...
\ifshowtrans %(校正 4/23 diningyo)
図~\ref{fig:logic}はこの組み合わせの表現の回路図を示します（信号は単一、もしくは複数のビットになり得る。Chiselの表現と回路図は同じになる）。
...
%%% FIXME -- somehow ~\ref{tab:operators} results in "2.2", not the expected number "2.1"
%%% for now manually embed the table number 2.1... 
表 2.1 に、演算子の完全なリストを示します（\myref{https://github.com/freechipsproject/chisel3/wiki/Builtin-Operators}{組み込み演算子}も参照）。
...
Verilogでは論理演算子間の優先順位がありますが、VHDLではこれらの演算子は同じ優先順位を持ち、左から右に評価されます。}
\fi
...
\begin{table}
 ...
 \label{tab:functions}
  \begin{tabular}{lll}
    ...
    メソッド & 処理 & データ型 \\
    \midrule
    \code{v.andR v.orR v.xorR} & リダクション演算 & UInt, SInt, returns Bool \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Chiselで定義されているハードウェアの関数（\code{v}に対しての呼び出し）}
\end{table}
\fi
...
Chiselには
% \jaenref{Dフリップフロップ}{https://ja.wikipedia.org/wiki/%E3%83%95%E3%83%AA%E3%83%83%E3%83%97%E3%83%95%E3%83%AD%E3%83%83%E3%83%97#D%E5%9E%8B%E3%83%95%E3%83%AA%E3%83%83%E3%83%97%E3%83%95%E3%83%AD%E3%83%83%E3%83%97}
\jaenref{Dフリップフロップ}{https://ja.wikipedia.org/wiki/フリップフロップ#D型フリップフロップ}
...
\ifshowtrans %(校正 7/24 mune10)
デフォルト\code{sbt}の検索対象は、ソースツリーの\code{main}以下のみで、\code{test}以下は含みません。
\footnotexbiased{これは、JavaやScalaではテストフォルダが単体テストしか含まず、\codefoot{main}をもつオブジェクトを含まない慣例からきています。}
しかしながら、Chiselのテスタは、ここで説明するように、\code{main}を含みながらも、ソースツリーの\code{test}以下に配置されます。
...
ハードウェア設計のテストは \emph{テストベンチ（test benches）}と呼ばれます。
...
\ifshowtrans %(校正 5/20 mune10)
\noindent Scalaの整数足し算をテストするためのミニマムテスト（テスト用のHello World）は次のようなものです。
\fi
...
例えば、教科書的なパイプライン構成を持つ RISC プロセッサの場合、約3000個の 4ビットLUT を使います。
...
\fi
\ifshowtrans %(校正)
\section{コンポーネントはChisel ではモジュール}
\fi
...
\ifshowtrans %(校正 10/10 mune10)
ALU は、図中の\code{A}と\code{B}の 2 つのデータ入力と、1 つの関数入力\code{fn}と出力の \code{Y}を持ちます。
...
% このデコーダの機能は、表~\ref{tab:decoder}のような真理値表として表現できます。
%%% FIXME
このデコーダの機能は、表5.1のような真理値表として表現できます。
\fi
...
図~\ref{fig:encoder}は、4ビットのワンホットな入力を2ビットのバイナリに変換したものを示しており、
% 表~\ref{tab:encoder}は、そのエンコードの真理値表です。
%%% FIXME
表5.2は、そのエンコードの真理値表です。
しかし、エンコーダは入力信号がワンホットな場合にのみ、期待通りに動作します。その他のすべての入力については、出力値は未定義となります。未定義の出力をもった機能を実装することは出来ないので、未定義となる入力のパターンを処理するためのデフォルト値を割り当てます。
\fi
...
レジスタコンストラクタの\code{RegInit}にパラメータとしてリセット値、例えばゼロを与えます。
...
\ifshowtrans %(校正 6/20 mune10 校正1 12/29 aki)
波形は、回路の動作をグラフィカルに把握するための優れたツールです。
...
ほとんどのサイクルで\code{enable}は\code{true}（High）で、レジスタの値は1クロックサイクルの遅延で入力に従っています。
...
この違反はフリップフロップの
\enref{メタスタビリティ（Metastability）}{https://en.wikipedia.org/wiki/Metastability_(electronics)}
を引き起こすことがあります。
...
入力シンクロナイザのChiselコードは、2つのレジスタをインスタンス化するワンライナー（1行コード）です。
\fi
...
これら2つの結果の唯一の違いは、\code{debounce A}の切り替わりが1サンプル周期遅れていることです。
しかし、これは通常は問題にはなりません。
\fi
...
サンプルタイミングは、~\ref{sec:gen:timing}項で行ったように、1サイクルのティック（\code{tick}）信号を出すカウンタで生成します。
\fi
...
\ifshowtrans %(校正 12/30 mune10)
入力信号が、時々ノイズを含んでいて、こうしたノイズを入力シンクロナイザやデバウンスユニットが意図せずにサンプリングしてしまう可能性があります。
...
アラームは\emph{クリア（clear）}信号でリセットできます。
\fi
...
以前にもこのChiselのワンライナー（1行コード）を見たことがあります：
\fi
...
\ifshowtrans %(校正 6/20 mune10 校正1 01/01 aki)
協調型FSMについて議論するために、我々は参考文献~\cite[17章]{dally:vhdl:2016}の、ライトフラッシャの例を使用します。
...
\ifshowtrans %(仮訳 12/20 校正1 01/01 aki)
仕様通りに単純に実装したFSM\footnote{状態遷移図は、参考文献~\cite[376ページ]{dally:vhdl:2016}に示されています。}は27の状態を持っており、
...
\ifshowtrans %(校正 12/30 diningyo)
Tickerクラスを継承して作成した異なるすべての論理は、単一のテストベンチを用いてテストできます。
...
\include{chisel-book-ja/profiles}

\include{chisel-book-ja/colophon}
