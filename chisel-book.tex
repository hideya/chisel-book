...
\ \\
\vspace{1cm}
{\usekomafont{title}\mdseries\Large 第二版(日本語版 RC 0.4)}
\cleardoublepage
\end{flushleft}
\newpage
\fi
...
\bigskip
%{\usekomafont{title}\huge Second Edition}\\
{\usekomafont{title}\huge 第二版(日本語版 RC 0.4)}\\
\bigskip
\bigskip
\bigskip
\bigskip
...
\fi
https://www.overleaf.com/project/5e60eac5de622d000111102e
\ifshoworiginal
...
例えば\code{1.U(32)}のような表現は32-bitの１を表す定義では\emph{ありません}。
...
そのテストとは、そのデザインから点滅する論理を削除し、１つのスイッチをLEDの出力に接続し、コンパイルとFPGAへの設定を行うことです。
...
テスタは\code{step(1)}でシミュレーションを１ステップ（= 1クロックサイクル）進めます。
...
関数で型パラメータを使ったときと同様に、モジュールのコンストラクタへ型パラメータ\code{T}を追加します。さらにこの型をパラメータの１つとするコンストラクタが必要になります。加えてこの例では、ルーターのポート数を可変にするための変数も追加してあります。
\fi
...
\ifshowtrans %(校正 12/23 diningyo)
この例では、ルーターの入力として、２つの異なる型を持つベクターを使用しました。１つはアドレス用のベクターで、もう１つはデータ用でパラメタライズされたものとなります。
よりエレガントな解決策は、次の例のようにこの２つのベクタ自体がパラメータ化された\code{Bundle}を持っていることです。
\fi
...
\ifshowtrans %(校正 12/31 mune10)
\longlist{code/uart_buffer.txt}{ready/valid インターフェースを持つ１バイトバッファ}{lst:uart:buffer}
\fi
